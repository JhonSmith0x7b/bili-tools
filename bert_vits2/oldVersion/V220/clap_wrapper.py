...
models = dict()
processor = ClapProcessor.from_pretrained("./bert_vits2/emotional/clap-htsat-fused")
...
def get_clap_audio_feature(audio_data, device=config.bert_gen_config.device):
    ...
    if device not in models.keys():
        models[device] = ClapModel.from_pretrained("./bert_vits2/emotional/clap-htsat-fused").to(
            device
        )
    with torch.no_grad():
        inputs = processor(
